...
+ And $C$ the number of individuals counted in the field
...
```
Among newly capture individuals ($m$), some are marked ($n_2$), others aren't
...
+ The proportion of marked individuals in second sample is $\displaystyle{\frac{m}{n_2}}$
...
---
## Lincoln-Petersen index requires individual indentification
...
$\hat{N}=\displaystyle{\frac{n_1}{\hat{p}}}$

with $\hat{p}=\displaystyle{\frac{m}{n_2}}$
...
Generalisation of Linclon-Petersen index to more than two sessions
...
---
# Fit a detection curve to the raw dat
...
+ Some strong assumptions :
    ...
    - Individuals have same detection probability (no heterogeneity)

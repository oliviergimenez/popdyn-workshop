...
setwd("~/Documents/ENSEIGNEMENT/WorkshopDynPop_2023/LiveDemos")
...
# load package (authors : Chris Stubben, Brook Milligan, Patrick Nantel) 
library(popbio)
library(ggplot2)
...
n1 <- grizzly$N[inityear] # initial population size in 1959
n1

T <-  50 # Time steps to project over

# change to 30 years, more reasonable
#T <-  30

# number of repetitions (pop. trajectories)
runs <- 500  

# Empty matrix to store resuts of population sizes     
stoch.pop <- matrix(NA,T,runs) 

stoch.pop[1,] <- n1 # initiate
...
dataproj <- data.frame(year = grizzly$year[inityear] : (grizzly$year[inityear] + (T-1)),
                       pop.mean= pop.mean,
                       low = ucl,
                       up = lcl,
...
t <-20 # number of time steps (years here) to project over
# run the projections
...
###### 1 - Include annual variation around mean values by drawing from truncated normal distributions (you can also use logit) ----
library(truncnorm)
